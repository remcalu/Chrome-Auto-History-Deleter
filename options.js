...
// Checks if the element has been clicked for the "Clear selected items" option 
function clickChecker(element) {
	console.log(element);
...
$('clearSelected').onclick = function () {
	let checkboxes = document.getElementsByClassName('labelClass');
	...
	chrome.storage.sync.get({
		...
		function (data) {
			for (i = 0; i < checkboxes.length; i++) {
				if (checkboxes[i].classList.contains('selected') == true) {
...
setTimeout(function () {
	for (i = 0; i < itemCount; i++) {
		temp = document.getElementById("item-" + i);
		clickChecker(temp);
	}
}, 500);